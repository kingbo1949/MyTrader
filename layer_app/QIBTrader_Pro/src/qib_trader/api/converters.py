...
from qib_trader.core.models import BarData, Interval
...
class IceConverter:
    ...
    def klines_to_df(klines: List[IBTrader.IKLine]) -> pd.DataFrame:
        ...
        # [保持高性能] 利用对象字典批量构造，避开 Python 显式循环
        # 注意：如果开启了 slots，__dict__ 可能不可用，届时需改用 dataclasses.asdict 或手动推导
    ...
    @staticmethod
    def df_to_bars(df: pd.DataFrame, symbol: str, interval_str: str) -> List[BarData]:
        """
