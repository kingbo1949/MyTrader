"""
MixUTurnBottomLongCall 策略：底部 UTurn 信号 → 直接开仓 Bull Call Spread → 持有至到期结算。
同时买入 long call（K1，较低行权价）和卖出 short call（K2，较高行权价），支付净权利金。
"""
import math
...
class MixUTurnBottomLongCall(Strategy):
    """
    底部 UTurn 期权策略（Bull Call Spread），触发信号后立即同时开两腿，持有至到期结算。
    params: risk_free_rate, option_days, entry_long, entry_short,
            expiry_weekday, min_drop_from_prev_close, cooldown_hours, max_positions
    """

    options_mode: bool = True
    initial_capital: float = 100_000.0

    entry_long: float = 0.5           # 买入腿 call delta（K1，较低行权价，靠近平值）
    entry_short: float = 0.3          # 卖出腿 call delta（K2，较高行权价，更 OTM）
    option_days: int = 7              # 期权最短到期天数
    expiry_weekday: int = 2           # 到期日星期几（0=周一...4=周五，-1=每日期权）
    risk_free_rate: float = 0.04
    strike_round: float = 100.0       # 行权价舍入精度
    min_drop_from_prev_close: float = 0.018
    ...
    require_lower_k1_than_existing: int = 0  # 1=连续开仓时新仓K1须低于所有现有仓位K1；0=不限制
    ...
    def _settle_spread(self, sp: dict, bar: BarData, close_reason: str = 'expiry') -> None:
        """到期结算。payoff = max(S-K1,0) - max(S-K2,0)；final_pnl = payoff - net_debit。"""
        S = bar.close_price
        long_settle  = max(S - sp['k1'], 0.0)   # 买入腿（K1）到期价值
        short_settle = max(S - sp['k2'], 0.0)   # 卖出腿（K2）到期价值
        payoff = long_settle - short_settle
        net_debit = sp['net_debit']
        final_pnl = payoff - net_debit
        # 入场时已支付 net_debit（LONG），结算时收到 payoff（SHORT）
        self._send_order(Direction.SHORT, payoff, 1, "MKT")
        self._record_position(sp, bar, close_reason, long_settle, short_settle, final_pnl)
        self.logger.info(
    ...
    def _record_position(self, sp: dict, bar: BarData, close_reason: str,
                         long_settle: float, short_settle: float, final_pnl: float) -> None:
    ...
    def _check_entry(self, bar: BarData) -> None:
        """直接开仓 Bull Call Spread：同时买入 K1 call 和卖出 K2 call。"""
        ...
        k1 = self._call_strike_by_delta(bar.close_price, T, self.entry_long,  iv_long)
        k2 = self._call_strike_by_delta(bar.close_price, T, self.entry_short, iv_short)
        # 连续开仓时，新仓 K1 必须低于所有现有仓位的 K1
        if self.require_lower_k1_than_existing and self._spreads and k1 >= min(sp['k1'] for sp in self._spreads):
            return
        p1 = bs_call_price(bar.close_price, k1, T, self.risk_free_rate, iv_long)
        p2 = bs_call_price(bar.close_price, k2, T, self.risk_free_rate, iv_short)
        net_debit = p1 - p2
        self._last_entry_dt = bar.datetime
        self._spreads.append({
            'entry_dt': bar.datetime, 'lock_dt': bar.datetime, 'lock_reason': 'direct_entry',
            'k1': k1, 'k2': k2,
            'long_prem': p1, 'short_prem': p2, 'net_debit': net_debit,
            'expiry': expiry,
        ...
        self.logger.info(
            f"[{bar.datetime}] 开仓BullCallSpread K1={k1} K2={k2} "
