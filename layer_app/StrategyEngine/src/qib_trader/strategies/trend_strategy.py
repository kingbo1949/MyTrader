...
class DoubleMAStrategy(Strategy):
    ...
    def on_init(self) -> None:
        ...
        """
        self.logger.info(f"初始化策略 {self.strategy_id}，参数: Fast={self.fast_window}, Slow={self.slow_window}")
    ...
    def logic(self, bar: BarData) -> None:
        ...
        # 4. 金叉死叉逻辑，根据 direction 决定操作
        if fast_ma > slow_ma:
            if self.direction == Direction.LONG and self.pos <= 0:
                self.logger.info(f"[{self.strategy_id}] 金叉开多")
                self.buy(bar.close_price, 1)
            elif self.direction == Direction.SHORT and self.pos < 0:
                self.logger.info(f"[{self.strategy_id}] 金叉平空")
                self.buy(bar.close_price, 1)

        elif fast_ma < slow_ma:
            if self.direction == Direction.SHORT and self.pos >= 0:
                self.logger.info(f"[{self.strategy_id}] 死叉开空")
                self.sell(bar.close_price, 1)
            elif self.direction == Direction.LONG and self.pos > 0:
                self.logger.info(f"[{self.strategy_id}] 死叉平多")
                self.sell(bar.close_price, 1)
