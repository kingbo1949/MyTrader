...
CVwMaGraph::CVwMaGraph(QCustomPlot* parent)
	:CAvgValueGraph(parent)
{
}

bool CVwMaGraph::MakeMap(const CodeStr& codeId, Time_Type timetype, const IBKLinePtrs& klines, int beginPos)
{
	...
	IBAvgValuePtrs avgValues = MakeAndGet_QDatabase()->GetVwMas(codeId, timetype, query);
