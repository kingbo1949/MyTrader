...
module IBTrader
{
	interface IQDatabase
	{
		...
		// 用tick更新数据库
		void		UpdateTickToDB(ITick tick, bool isIndex);							// 把TICK更新到数据库各个表中
		...
		// KLine表
		void		UpdateKLine(string codeId, bool isIndex, ITimeType timeType, IKLine kline);
		void		UpdateKLines(string codeId, bool isIndex, ITimeType timeType, IKLines klines);
		...
		void		GetInvalidKLines(string codeId, bool isIndex, ITimeType timeType, out IKLines klines);	// 得到非法的BAR为清理数据库做准备
